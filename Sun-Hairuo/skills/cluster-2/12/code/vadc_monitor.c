...
void adc_task(){
  ...
  while (1) {
    if (adc_flag) {
      ...
      printf("%d Raw: %d\tVoltage: %dmV\n", (adc_index + 1), adc_reading, voltage);
      voltage_sum += voltage;

      if (adc_index == 9){
        vol = (int)(voltage_sum / 10);// update global voltage varaible "vol"
        // printf("voltage_sum: %d", voltage_sum);
        printf("Mean Voltage: %dmV\n\n", vol);
        voltage_sum = 0;
        adc_index = 0;
      }
      else {
...
static void alpha_task() {
    ...
    while (1) {
        ...
        if (alpha_flag){
          ...
          int index = 3;
          uint16_t temp_vol = vol;
          while (temp_vol > 0) {
            displaybuffer[index] = alphafonttable[(temp_vol % 10)];  // LSB to MSB
            temp_vol /= 10;
            index--;
          }
          alpha_flag = false;
      ...
      vTaskDelay(10 * TIMER_INTERVAL_SEC * 1000 * 0.9 / portTICK_RATE_MS);
    }
...
void app_main(void)
{
  ...
  xTaskCreate(adc_task,"adc task", 4096, NULL, MAX_PRIORITIES-1, NULL);
