...
void ir_adc_task(){
  ...
  while (1) {
    if (adc_flag) {
      ...
      // uint32_t voltage = adc_reading * (5 / 1024);
      // printf("adc_reading: %d", adc_reading);
      // double ir_distance = 65 * pow(voltage, -1.10);
      double ir_distance = -0.0619047619048* voltage + 125.238095238;
      // printf("ir: distance:%.2f", ir_distance);
      printf("\nRaw ADC Reading: %d\tVout: %dmV\n", adc_reading, voltage);
      if (ir_distance < 20){
        ir_distance = 20;
        printf("\ndistance < 20.\n");
      }
        ...
        printf("IR Range sensor distance: %.2fcm\n\n", ir_distance);
      }
