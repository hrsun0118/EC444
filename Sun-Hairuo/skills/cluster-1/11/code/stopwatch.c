...
bool flag = false;   // global variable "flag": true: up; false: down
int count = 0;
...
static void task_C() {
    ...
    while (1) {
        // get an array of digits for "count_up"
        ...
        if (count == 0){
            displaybuffer[3] = 0b0000000000111111;
        }

        int temp_num = count;  // for dividing to get each digits
        int index = 0;
        int remainder;
        while(temp_num > 0){
          remainder = temp_num % 10;
          temp_num = temp_num / 10;
          displaybuffer[3-index] = alphafonttable[remainder];
          index++;
        }
