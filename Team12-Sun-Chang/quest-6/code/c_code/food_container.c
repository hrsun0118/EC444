...
// For UDP Client
#define ESP32_1_IP_ADDR       "10.0.0.230"
#define ESP32_1_PORT          1136    // ESP32 1 Listen Port
static const char *TAG = "ESP32 2";
...
void getAccel(float * xp, float *yp, float *zp) {
 ...
 printf("X: %.2f \t Y: %.2f \t Z: %.2f\t", *xp, *yp, *zp);
}
...
static void udp_client_task(void *pvParameters)
{
    ...
    while (1) {
        ...
        dest_addr.sin_addr.s_addr = inet_addr(ESP32_1_IP_ADDR);
        dest_addr.sin_family = AF_INET;
        dest_addr.sin_port = htons(ESP32_1_PORT);
        addr_family = AF_INET;
        ...
        ESP_LOGI(TAG, "Socket created, sending to %s:%d", ESP32_1_IP_ADDR, ESP32_1_PORT);

        while (1) {
          if (udp_flag){
            sprintf(payload, "%.2f,%.2f,%.2f,%.2f", xVal, yVal, zVal, tempVal);
            int err = sendto(sock, &payload, strlen(payload), 0, (struct sockaddr *)&dest_addr, sizeof(dest_addr));
            if (err < 0) {
                ESP_LOGE(TAG, "Error occurred during sending: errno %d", errno);
                break;
            }
            ESP_LOGI(TAG, "Message sent is: %s", payload);
            udp_flag = false;
          }
            vTaskDelay(0.9 * TIMER_INTERVAL_SEC * 1000 / portTICK_PERIOD_MS);
        }
...
void app_main() {
  ...
  xTaskCreate(test_adxl343,"test_adxl343", 4096, NULL, MAX_PRIORITIES-1, NULL);
  xTaskCreate(thermistor_adc_task,"thermistor_adc_task", 4096, NULL, MAX_PRIORITIES-1, NULL);
  xTaskCreate(udp_client_task, "udp_client task", 4096, NULL, MAX_PRIORITIES-1, NULL);
